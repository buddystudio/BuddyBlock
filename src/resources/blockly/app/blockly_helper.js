/**
 * Execute the user's code. Just a quick and dirty eval. No checks for infinite
 * loops, etc.
 */
function runJS() {
...
/**
 * Save blocks to local file. better include Blob and FileSaver for browser
 * compatibility
 */
function save() {
  var xml = Blockly.Xml.workspaceToDom(Blockly.mainWorkspace);
 ...
 * 
 * @param {!Element}
 *            element Element upon which to listen.
 * @param {string}
 *            name Event name to listen to (e.g. 'mousedown').
 * @param {!Function}
 *            func Function to call when event is triggered. W3 browsers will
 *            call the function with the event object as a parameter, MSIE will
 *            not.
 */
...
function uploadCode(code, callback) {
    ...
    request.onreadystatechange = function() {
        ...
        var status = parseInt(request.status); // HTTP response status, e.g.,
												// 200 for "200 OK"
        var errorInfo = null;
        switch (status) {
        case 200:
