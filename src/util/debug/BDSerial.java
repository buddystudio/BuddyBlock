...
public class BDSerial implements SerialPortEventListener 
{
	SerialPort port;
	...
	public BDSerial(boolean monitor) throws Exception {
		this(Preferences.get("serial.port"), Preferences.getInteger("serial.debug_rate"),
				Preferences.get("serial.parity").charAt(0), Preferences.getInteger("serial.databits"),
				new Float(Preferences.get("serial.stopbits")).floatValue());
		this.monitor = monitor;
	}

	public BDSerial() throws Exception {
		this(Preferences.get("serial.port"), Preferences.getInteger("serial.debug_rate"),
				Preferences.get("serial.parity").charAt(0), Preferences.getInteger("serial.databits"),
				new Float(Preferences.get("serial.stopbits")).floatValue());
	}

	public BDSerial(int irate) throws Exception {
	...
	public BDSerial(String iname, int irate) throws Exception {
		this(iname, irate, Preferences.get("serial.parity").charAt(0),
			    Preferences.getInteger("serial.databits"), 
			    new Float(Preferences.get("serial.stopbits")).floatValue());
	}

	public BDSerial(String iname) throws Exception {
	...
	public static boolean touchPort(String iname, int irate) throws Exception {
		SerialPort port;
		...
		try {
			...
			throw new Exception(String
					.format("Serial port %s already in use. Try quitting any programs that may be using it.", iname));
		} catch (Exception e) {
			throw new Exception(String.format("Error touching serial port %s.", iname), e);
		}
		return result;
	}

	public BDSerial(String iname, int irate, char iparity, int idatabits, float istopbits) throws Exception {
		...
		try {
			...
			throw new Exception(String
					.format("Serial port %s already in use. Try quiting any programs that may be using it.", iname));
		} catch (Exception e) {
			throw new Exception(String.format("Error opening serial port %s.", iname), e);
		}

		if (port == null) {
			throw new Exception(String.format(
					"Serial port %s not found. Did you select the right one from the Tools > Serial Port menu?",
					iname));
		}
	}
	...
	public void addListener(BDMessageConsumer consumer) {
	...
	static public void errorMessage(String where, Throwable e) {
		System.err.println(String.format("Error inside Serial.{0}()", where));
		e.printStackTrace();
	}
}
